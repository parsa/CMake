...
void cmRedoTargetGenerator::Generate()
{
  ...
  for (cmSourceFile const* source : sources) {
    ...
    rulePlaceholderExpander->ExpandRuleVariables(this->LocalGenerator, compileCmd,
                                                 vars);
    bool const inTryCompile =
      this->Makefile->GetCMakeInstance()->GetIsInTryCompile();
    // Emit a Makefile-style depfile. We store it next to the object as $1.d.
    // Skip depfile tracking in try_compile builds: these build trees are always
    // fresh and the dependency scanning/registration overhead can dominate
    // configure time.
    if (!inTryCompile) {
    ...
    // In try_compile build trees we can skip explicit dependency registration
    // for sources/custom outputs because the tree is always fresh.
    if (!inTryCompile) {
    ...
    if (!inTryCompile) {
      ...
      lines.emplace_back("    \"$cmake_cmd\" -E cmake_depfile_deps \"$(pwd)\" \"$dep\" \"$deps\"");
